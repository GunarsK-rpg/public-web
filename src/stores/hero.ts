...
export const useHeroStore = defineStore('hero', () => {
  ...
  function applyCulturalExpertise(cultureId: number) {
    ...
    const expertiseType = classifiers.getByCode(classifiers.expertiseTypes, 'cultural');
    if (!expertiseType) return;

    const expertise = classifiers.expertises.find(
      (e) => e.expertiseTypeId === expertiseType.id && e.name === culture.name
    );
    if (expertise) {
      addExpertise(expertise.id, { sourceType: 'culture', sourceId: cultureId });
    }
  }
